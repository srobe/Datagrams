...
def find_index(array, value, default=None):

    mindiff = None

    for indx, val in enumerate(array):

        if val == value: return indx

        diff = abs(val-value)

        if mindiff == None: mindiff = diff
...
def stnSample(f,V,stnLon,stnLat,tyme,options,squeeze=True):
    ...
    if V.km > 0:
        ...
        lev1, lev2 = (f.levs[0], f.levs[V.km-1])

        if options.lev1: lev1 = options.lev1
        if options.lev2: lev2 = options.lev2

        l1  = find_index(f.levs[0:V.km],lev1)
        l2  = find_index(f.levs[0:V.km],lev2)
        ...
        try:
            ...
            if nz>0:
                z = f.interp(name,stnLon,stnLat,tyme=t,algorithm=options.algo,
                         Transpose=True,squeeze=squeeze,kbeg=l1+1,kount=nz)
            else:
                z = f.interp(name,stnLon,stnLat,tyme=t,algorithm=options.algo,
...
def writeXLS ( stnName, stnLon, stnLat, tyme, f, options,
    ...
    if f.km > 0:
        ...
        lev1, lev2 = (f.levs[0], f.levs[f.km-1])

        if options.lev1: lev1 = options.lev1
        if options.lev2: lev2 = options.lev2

        l1  = find_index(f.levs,lev1)
        l2  = find_index(f.levs,lev2)

        if l2 < l1: l1, l2 = (l2, l1)

        nz_ = l2 - l1 + 1

    ns_, nt_ = ( len(stnLon), len(tyme) )
...
def writeNC ( stnName, stnLon, stnLat, tyme, f, options,
    ...
    stnName_.axis = 'e'
    stntmp = zeros((ns_,50),dtype='S1')
    for i in range(ns_):
        stntmp[i][:] = list('%-50s'%stnName[i])
    stnName_[:] = stntmp[:]
...
#   --------------------------
    ...
    parser.add_option("-s", "--lev1", type=float,
                      dest="lev1", default=None,
                      help="Starting pressure level.")

    parser.add_option("-e", "--lev2", type=float,
                      dest="lev2", default=None,
                      help="Ending pressure level.")
